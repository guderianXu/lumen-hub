...
def _ordered_fan_roles(fans: list[object]) -> list[str]:
...
class StressTestPanel(QWidget):
    ...
    def __init__(self) -> None:
        super().__init__()
        self.setObjectName("FanStressPanel")
        self.burner = None
        ...
        for column, (kind, (title_text, tool_text)) in enumerate(self._LABELS.items()):
            card = QFrame()
            card.setObjectName("FanStressCard")
            ...
            state = QLabel("未运行")
            state.setObjectName("FanStressState")
    ...
    def _set_kind_state(self, kind: str, running: bool) -> None:
        if kind not in self._buttons:
            return
        button = self._buttons[kind]
        button.blockSignals(True)
        button.setChecked(running)
        button.setText("停止" if running else "启动")
        button.setObjectName("DangerButton" if running else "PrimaryButton")
...
class EmbeddedFanControlPanel(QWidget):
    ...
    def __init__(self, fan_slider_cls, parent=None) -> None:  # noqa: ANN001
        ...
        toolbar = QFrame()
        toolbar.setObjectName("FanControlToolbar")
        top = QGridLayout(toolbar)
        ...
        self._show_active_only_cb.setMinimumWidth(118)
        self._show_active_only_cb.toggled.connect(self._apply_filter)
        filter_label = QLabel("筛选")
        filter_label.setObjectName("SectionLabel")
        top.addWidget(self._info_label, 0, 0, 2, 1)
        top.addWidget(filter_label, 0, 1)
        top.addWidget(self._role_filter_combo, 1, 1)
        top.addWidget(self._show_active_only_cb, 1, 2, Qt.AlignmentFlag.AlignLeft | Qt.AlignmentFlag.AlignVCenter)
    ...
    def _make_fan_row(self, fan, temp_sensors) -> QFrame:  # noqa: ANN001
        ...
        identity_pane = QWidget()
        identity_pane.setObjectName("FanControlIdentityPane")
        identity_layout = QVBoxLayout(identity_pane)
        ...
        write_pane = QWidget()
        write_pane.setObjectName("FanControlWritePane")
        write_layout = QVBoxLayout(write_pane)
        write_layout.setContentsMargins(0, 0, 0, 0)
        write_layout.setSpacing(6)
        write_title = QLabel("PWM 输出")
        write_title.setObjectName("FanControlPaneTitle")
        write_layout.addWidget(write_title)
        ...
        write_layout.addWidget(slider)
        layout.addWidget(identity_pane, 0, 0)
        layout.addWidget(write_pane, 0, 1)
...
class EmbeddedFanBindingPanel(QWidget):
    def __init__(self, parent=None) -> None:  # noqa: ANN001
        ...
        toolbar = QFrame()
        toolbar.setObjectName("FanBindingToolbar")
        top = QGridLayout(toolbar)
        top.setContentsMargins(12, 10, 12, 10)
        top.setHorizontalSpacing(10)
        top.setVerticalSpacing(6)
        self._info_label = QLabel("加载后按风扇角色显示温度源绑定")
        self._info_label.setObjectName("FieldHint")
        self._info_label.setWordWrap(True)
        self._role_filter_combo = QComboBox()
        self._role_filter_combo.setMinimumWidth(150)
        ...
        self._scroll = QScrollArea()
        self._scroll.setObjectName("FanBindingScrollArea")
        self._scroll.setWidgetResizable(True)
        self._container = QWidget()
        self._container.setObjectName("FanBindingContainer")
        self._binding_layout = QVBoxLayout(self._container)
    ...
    def populate_fans(self, fans, temp_sensors) -> None:  # noqa: ANN001
        ...
        for role in roles:
            ...
            section_layout = section.layout()
            for fan in role_fans:
                row = self._make_binding_row(fan, temp_sensors)
                section_layout.addWidget(row)
                name = str(getattr(fan, "name", ""))
                self._rows[name] = row
                self._fan_roles[name] = role
                self._section_rows.setdefault(role, []).append(row)
            self._binding_layout.addWidget(section)
            self._section_widgets[role] = section
    ...
    def _make_section(self, role: str, count: int) -> QFrame:
        section = QFrame()
        section.setObjectName("FanBindingGroup")
        layout = QVBoxLayout(section)
        layout.setContentsMargins(14, 12, 14, 14)
        layout.setSpacing(10)
        title = QLabel(f"温度绑定 · {_fan_role_compact_label(role)} · {count} 通道")
        title.setObjectName("SectionLabel")
    ...
    def _make_binding_row(self, fan, temp_sensors) -> QFrame:  # noqa: ANN001
        ...
        identity_pane = QWidget()
        identity_pane.setObjectName("FanBindingIdentityPane")
        identity_layout = QVBoxLayout(identity_pane)
        identity_layout.setContentsMargins(0, 0, 0, 0)
        ...
        picker_pane = QWidget()
        picker_pane.setObjectName("FanBindingPickerPane")
        picker_layout = QGridLayout(picker_pane)
        ...
        picker_layout.addWidget(bind_label, 0, 0)
        picker_layout.addWidget(combo, 0, 1)
        picker_layout.addWidget(hint_label, 1, 0, 1, 2)
        picker_layout.setColumnStretch(1, 1)
        bind_layout.addWidget(identity_pane, 0, 0)
        bind_layout.addWidget(picker_pane, 0, 1)
        bind_layout.setColumnMinimumWidth(0, 220)
        bind_layout.setColumnStretch(0, 0)
...
class EmbeddedProfileEditor(QWidget):
    ...
    def __init__(self, profile_manager, curve_editor_cls, fan_curve_cls, profile_cls, parent=None) -> None:  # noqa: ANN001
        ...
        sidebar = QFrame()
        sidebar.setObjectName("FanProfileSidebar")
        ...
        self._profile_state_label.setWordWrap(True)
        self._profile_state_label.setAlignment(Qt.AlignmentFlag.AlignRight | Qt.AlignmentFlag.AlignVCenter)
        header_layout.addWidget(self._profile_state_label, 1)
        top_layout.addLayout(header_layout)

        profile_layout = QGridLayout()
        profile_layout.setHorizontalSpacing(10)
        profile_layout.setVerticalSpacing(8)
        profile_label = QLabel("当前编辑")
        profile_label.setObjectName("FieldHint")
        ...
        profile_layout.addWidget(self._profile_combo, 0, 1, 1, 3)
        ...
        for index, button in enumerate((self._btn_new, self._btn_delete, self._btn_save, self._btn_activate)):
            profile_layout.addWidget(button, 1, index)
            profile_layout.setColumnStretch(index, 1)
        top_layout.addLayout(profile_layout)
        sidebar_layout.addWidget(top_card)
        ...
        status_layout.addWidget(self._cpu_temp_label, 0, 0)
        status_layout.addWidget(self._gpu_temp_label, 0, 1)
        status_layout.addWidget(self._fan_rpm_label, 1, 0, 1, 2)
        ...
        curve_panel = QFrame()
        curve_panel.setObjectName("FanCurveEditorPanel")
        ...
        curve_title = QLabel("曲线编辑")
        curve_title.setObjectName("SectionLabel")
        curve_hint = QLabel("CPU/GPU 曲线分开编辑，保存后再激活配置。")
        curve_hint.setObjectName("FieldHint")
        curve_hint.setAlignment(Qt.AlignmentFlag.AlignRight | Qt.AlignmentFlag.AlignVCenter)
        curve_header.addWidget(curve_title)
        curve_header.addWidget(curve_hint, 1)
        curve_layout.addLayout(curve_header)

        self._curve_tabs = QTabWidget()
        self._curve_tabs.setObjectName("FanProfileCurveTabs")
        self._curve_tabs.setMinimumHeight(540)
        ...
        curve_layout.addWidget(self._curve_tabs, 1)

        layout.addWidget(sidebar, 0, 0)
        layout.addWidget(curve_panel, 0, 1)
        layout.setColumnStretch(0, 0)
        layout.setColumnStretch(1, 1)
...
class FanControlHostPage(QWidget):
    ...
    def __init__(
        ...
        header_panel = QFrame()
        header_panel.setObjectName("FanCommandPanel")
        header_row = QGridLayout(header_panel)
        header_row.setContentsMargins(16, 14, 16, 14)
        header_row.setHorizontalSpacing(14)
        ...
        header_row.addLayout(title_box, 0, 0)
        ...
        action_row.addWidget(self.driver_probe_button)
        ...
        action_row.addWidget(self.enable_control_button)
        header_row.addLayout(action_row, 0, 1)

        self.status_label = QLabel("尚未加载风扇监控。加载只读监控不会写入 PWM。")
        self.status_label.setObjectName("FieldHint")
        self.status_label.setWordWrap(True)
        header_row.addWidget(self.status_label, 1, 0, 1, 2)
        header_row.setColumnStretch(0, 1)
        header_row.setColumnStretch(1, 0)
        self.layout.addWidget(header_panel)
        ...
        summary_cards = (
            ...
            ("风扇通道", self.fan_count_value),
            ("传感器", self.sensor_count_value),
            ("当前策略", self.active_profile_value),
            ("PWM 权限", self.permission_value),
        )
        for index, (label, value) in enumerate(summary_cards):
            summary.addWidget(self._summary_card(label, value), index // 3, index % 3)
        ...
        self.binding_layout = QVBoxLayout(self.binding_tab)
        self.binding_layout.setContentsMargins(0, 0, 0, 0)
        self.binding_layout.setSpacing(12)
        self.binding_placeholder = self._tab_placeholder("加载后显示每个 PWM 通道的温度源绑定")
        self.binding_layout.addWidget(self.binding_placeholder, 1)
        ...
        charts_panel = QFrame()
        charts_panel.setObjectName("FanChartPanel")
        charts_layout = QGridLayout(charts_panel)
        charts_layout.setContentsMargins(16, 14, 16, 16)
        charts_layout.setHorizontalSpacing(12)
        charts_layout.setVerticalSpacing(12)
        self.channel_cards_page = QWidget()
        channel_cards_layout = QVBoxLayout(self.channel_cards_page)
        ...
        for index, role in enumerate(("CPU 风扇", "水泵/AIO", "机箱风扇", "GPU 风扇")):
            ...
            role_metrics_layout.addWidget(card, index // 2, index % 2)
        ...
        visual_layout.addWidget(live_overview_page, 1, 0, 1, 2)
        ...
        self.charts_layout.addWidget(charts_panel, 1)
        ...
        permission_content = QWidget()
        permission_content.setObjectName("FanPermissionContent")
        permission_content_layout = QGridLayout(permission_content)
        permission_content_layout.setContentsMargins(0, 0, 0, 0)
        permission_content_layout.setHorizontalSpacing(12)
        permission_content_layout.setVerticalSpacing(12)

        permission_panel = QFrame()
        permission_panel.setObjectName("FanPermissionPanel")
        self.permission_status_panel = permission_panel
        permission_panel.setMinimumWidth(300)
        permission_panel.setMaximumWidth(440)
        permission_layout = QVBoxLayout(permission_panel)
        permission_layout.setContentsMargins(14, 12, 14, 12)
        permission_layout.setSpacing(12)
        ...
        self.permission_session_value = QLabel("普通用户" if os.geteuid() else "root")
        self.permission_pwm_files_value = QLabel(self.permission_value.text())
        self.permission_profile_value = QLabel(self._profile_config_permission_summary())
        permission_status = QWidget()
        permission_status.setObjectName("FanPermissionStatus")
        permission_status_layout = QGridLayout(permission_status)
        permission_status_layout.setContentsMargins(0, 0, 0, 0)
        permission_status_layout.setHorizontalSpacing(8)
        permission_status_layout.setVerticalSpacing(8)
        for index, (label, value) in enumerate(
            (
                ("会话", self.permission_session_value),
                ("PWM 文件", self.permission_pwm_files_value),
                ("策略目录", self.permission_profile_value),
            )
        ):
            permission_status_layout.addWidget(self._permission_status_chip(label, value), index, 0)
            permission_status_layout.setRowStretch(index, 1)

        permission_actions = QWidget()
        permission_actions.setObjectName("FanPermissionActions")
        ...
        for index, button in enumerate(
            ...
            permission_actions_layout.addWidget(button, index // 2, index % 2)
            permission_actions_layout.setColumnStretch(index % 2, 1)
        self.permission_detail_text = QTextEdit()
        self.permission_detail_text.setReadOnly(True)
        self.permission_detail_text.setMinimumHeight(300)
        self.permission_wizard_text = QTextEdit()
        self.permission_wizard_text.setReadOnly(True)
        self.permission_wizard_text.setMinimumHeight(300)
        self.permission_info_tabs = QTabWidget()
        self.permission_info_tabs.setObjectName("FanPermissionInfoTabs")
        self.permission_info_tabs.setMinimumHeight(360)
        permission_detail_panel = QFrame()
        permission_detail_panel.setObjectName("FanPermissionDetailPanel")
        self.permission_detail_panel = permission_detail_panel
        permission_detail_panel_layout = QVBoxLayout(permission_detail_panel)
        permission_detail_panel_layout.setContentsMargins(14, 12, 14, 14)
        permission_detail_panel_layout.setSpacing(10)
        permission_detail_title = QLabel("权限明细与诊断")
        permission_detail_title.setObjectName("SectionLabel")
        permission_detail_hint = QLabel("左侧只处理授权动作，这里单独显示文件状态、修复命令和诊断建议。")
        permission_detail_hint.setObjectName("FieldHint")
        permission_detail_hint.setWordWrap(True)
        ...
        permission_layout.addWidget(permission_hint)
        permission_layout.addWidget(permission_status)
        permission_layout.addWidget(permission_actions)
        permission_layout.addStretch(1)
        permission_detail_panel_layout.addWidget(permission_detail_title)
        permission_detail_panel_layout.addWidget(permission_detail_hint)
        permission_detail_panel_layout.addWidget(self.permission_info_tabs, 1)
        permission_content_layout.addWidget(permission_panel, 0, 0)
        permission_content_layout.addWidget(permission_detail_panel, 0, 1)
        permission_content_layout.setColumnStretch(0, 0)
        permission_content_layout.setColumnStretch(1, 1)
        self.permission_layout.addWidget(permission_content, 1)
        ...
        self.channel_detail_tabs.addTab(self.channel_cards_page, "实时卡片")
        ...
        self.workspace_tabs.addTab(self.binding_tab, "绑定")
    ...
    def release(self) -> None:
        ...
        self.binding_placeholder.setVisible(True)
    ...
    def _workspace_tab_changed(self, index: int) -> None:
        ...
        if widget is self.overview_tab:
            height = 520
        elif widget is self.charts_tab:
            height = 560
        elif widget is self.control_tab:
            height = 680
        elif widget is self.binding_tab:
            height = 620
        elif widget is self.strategy_tab:
            height = 760 if self.strategy_tabs.currentWidget() is self.strategy_editor_tab else 400
    ...
    def _build_embedded_ui(self, modules: dict[str, ModuleType | None]) -> None:
        ...
        try:
            ...
            fan_binding = EmbeddedFanBindingPanel()
            ...
            monitor.fan_rpm_updated.connect(
                lambda name, rpm: fan_binding.update_fan_rpm(self._display_fan_name(name), rpm)
            )
            ...
            self._populate_fan_binding(fan_binding, monitor)
    ...
    def _mount_embedded_widgets(self, *, fan_control, fan_binding, profile_editor, history_view) -> None:  # noqa: ANN001
        self._clear_embedded_widgets()
        self.control_placeholder.setVisible(False)
        self.binding_placeholder.setVisible(False)
        self.strategy_placeholder.setVisible(False)
        self.history_placeholder.setVisible(False)

        self._prepare_profile_editor_for_embedding(profile_editor)
        self.control_layout.addWidget(fan_control, 1)
        self.binding_layout.addWidget(fan_binding, 1)
        ...
        self._embedded_widgets = [fan_control, fan_binding, profile_editor, history_view]
        self.workspace_tabs.setCurrentWidget(self.overview_tab)
        self._tabs = None

    def _prepare_profile_editor_for_embedding(self, profile_editor) -> None:  # noqa: ANN001
        profile_editor.setMinimumHeight(640)
    ...
    def _populate_fan_binding(self, fan_binding, monitor) -> None:  # noqa: ANN001
        if getattr(fan_binding, "_rows", None):
            return
        temp_sensors = [sensor for sensor in monitor.sensors if getattr(sensor, "unit", "") == "°C"]
        fan_binding.populate_fans(self._controllable_display_fans(), temp_sensors)
    ...
    def _update_permission_summary(self) -> None:
        self.permission_value.setText(self._permission_summary_text())
        if hasattr(self, "permission_session_value"):
            self.permission_session_value.setText("root" if os.geteuid() == 0 else "普通用户")
        if hasattr(self, "permission_pwm_files_value"):
            self.permission_pwm_files_value.setText(self.permission_value.text())
        if hasattr(self, "permission_profile_value"):
            self.permission_profile_value.setText(self._profile_config_permission_summary())
    ...
    def _permission_status_chip(self, title: str, value: QLabel) -> QFrame:
        chip = QFrame()
        chip.setObjectName("FanPermissionStatusChip")
        chip.setMinimumHeight(72)
        chip_layout = QVBoxLayout(chip)
        chip_layout.setContentsMargins(10, 8, 10, 8)
        chip_layout.setSpacing(4)
        title_label = QLabel(title)
        title_label.setObjectName("FanCardMeta")
        value.setObjectName("FanPermissionStatusValue")
        value.setWordWrap(True)
        chip_layout.addWidget(title_label)
        chip_layout.addWidget(value, 1)
        return chip
