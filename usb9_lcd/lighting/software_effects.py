...
def _star_frame(count: int, frame_index: int, base: Rgb, offset: int) -> list[Rgb]:
    ...
    for index in range(count):
        ...
        if noise > 238:
            ...
            intensity = 0.03 if _noise(index + offset, 0, 97) > 210 else 0.0
        frame.append(_scale(base, intensity))
    return frame
...
def _scan_frame(count: int, frame_index: int, base: Rgb, offset: int, total: int) -> list[Rgb]:
    ...
    frame: list[Rgb] = []
    for index in range(count):
        distance = abs((index + offset) - position)
        intensity = max(0.0, 1.0 - distance / max(1, width))
        frame.append(_scale(base, intensity ** 1.4))
    return frame
...
def _visor_frame(count: int, frame_index: int, base: Rgb, offset: int, total: int) -> list[Rgb]:
    width = max(4, total // 5)
    cycle = total + width
    start = (frame_index * 2) % cycle - width
    frame: list[Rgb] = []
    for index in range(count):
        global_index = index + offset
        ...
        if 0 <= local <= width:
            ...
            frame.append(_scale(color, intensity))
        else:
            frame.append((0, 0, 0))
    return frame
