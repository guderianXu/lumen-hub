...
- 已有 `lianli-validation-gate`，用于把 Windows 抓包缺口、接收器实机证据和 bind/unbind 风险复核汇总成一个只读 gate。
...
- [x] 新增 `lianli-validation-gate`，用于把 `capture-gap-report`、`receiver-evidence-report` 和 `receiver-pairing-risk-report` 合并成一个 readiness JSON，优先暴露 blocker、warning 和下一步命令。
