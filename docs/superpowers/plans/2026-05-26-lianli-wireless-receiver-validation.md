...
- 已有 `windows-capture-runbook`，用于把当前抓包缺口转换成 Windows VM 操作清单、验收条件、tshark 导出命令、sidecar 记录模板和 Linux 后处理命令。
...
6. 生成一份可回传分析的证据清单：
   ...
   ```
7. 生成一份总 gate，确认当前到底卡在 Windows 抓包、接收器证据还是安全写入阶段：
   ```bash
   python tools/lianli_wireless_probe.py \
     --save-json .cache/lianli/validation-gate.json \
     lianli-validation-gate \
     --capture-dir .cache/lianli/captures \
     --hardware-dir .cache/lianli/hardware \
     --artifact-dir .cache/lianli \
     --capture-base lianli-v2117
   ```
8. 也可以在 GUI 里点“汇总实验”，选择 `.cache/lianli/hardware`，看实验流程里的“下一步”提示；如果只有一个可用 MAC，GUI 会自动填入目标 MAC。
9. 如果需要分步排查，再运行底层命令：
   ```bash
...
`capture-set-report.capture_note_context_summary.status` 应该是
`consistent-target-context`；如果是 `target-context-conflict`，先修正对应
sidecar，不能把冲突的 `common_target_args` 用于后续 packet compare。
